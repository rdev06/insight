...
export default async function (axios, transports, option) {
  ...
  axios.interceptors.request.use(async function (req) {
    ...
    const saveReq = omitAssign(req, toDeleteFromReq, {
      body: (req.body instanceof Buffer || req.body instanceof ReadableStream) ? 'stream' : req.body
    });
    saveReq.method = saveReq.method.toUpperCase();
    Promise.all(transports.map((transport) => transport.create({ _id: req._id, req: saveReq, timestamp: req.time.startTime })));
    return req;
  });
