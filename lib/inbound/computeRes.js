...
export default async function (_id, timestamp, req, res, _chunk) {
  ...
  const toUpdate = {
    timestamp, // for timesereis database
  ...
  if (returnType?.[0] === 'application/json' && !!_chunk) {
    toUpdate.res.body = JSON.parse(_chunk);
  } else {
    toUpdate.res.body = _chunk.toString(returnType?.[1] || 'utf-8');
  }
  toUpdate.res.headers = omitAssign(toUpdate.res.headers, this.resHeaderBlacklist)
  for (const k of this.whitelist) {
    if(req.hasOwnProperty(k)){
      if(!toUpdate.hasOwnProperty('req')) toUpdate.req = {};
