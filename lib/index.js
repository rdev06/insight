...
export default class Insight {
  ...
  async init(transports = []) {
    ...
    const trFn = (name, ttlfield, expire) => Promise.all(transports.map((tr) => tr(name, ttlfield, expire)));
    const CrashTr = await trFn(this.option.Crash?.name || 'Crash', 'timestamp', this.option.Crash?.expire);
    this.InboundTr = await trFn(this.option.Inbound?.name || 'Inbound', 'timestamp', this.option.Inbound?.expire);
    this.AxiosTr = await trFn(this.option.Outbound?.name || 'OutBound', 'time.startTime', this.option.OutBound?.expire);
    this.CustomEvTr = await trFn(this.option.Custom?.name || 'Custom', 'timestamp', this.option.Custom?.expire);
    this.TraceTr = await trFn(this.option.Trace?.name || 'Trace', 'timestamp', this.option.Trace?.expire);
