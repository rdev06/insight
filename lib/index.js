...
export default class Insight {
  ...
  async init(transports = []) {
    ...
    const CrashTr = await trFn(this.option.Crash?.name || 'Crash',  {field: 'timestamp', expireAfterSeconds: this.option.Crash?.expire});
    this.InboundTr = await trFn(this.option.Inbound?.name || 'Inbound',  {field: 'timestamp', expireAfterSeconds: this.option.Inbound?.expire});
    this.AxiosTr = await trFn(this.option.Outbound?.name || 'OutBound',  {field: 'timestamp', expireAfterSeconds: this.option.OutBound?.expire});
    this.CustomEvTr = await trFn(this.option.Custom?.name || 'Custom',  {field: 'timestamp', expireAfterSeconds: this.option.Custom?.expire});
    this.TraceTr = await trFn(this.option.Trace?.name || 'Trace',  {field: 'timestamp', expireAfterSeconds: this.option.Trace?.expire});
